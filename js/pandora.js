javascript:(function() {
    ...
    function updateSongStatus() {
        ...
        if ($artistName.length && $songName.length && $username.length && $artistName.text() !== 'audioad' && $artistName.text() !== 'ad' && $username.text() !== '' && $artistName.text() !== '' && $songName.text() !== '' && (currentSongID !== songID))
        {
            var payload = {
